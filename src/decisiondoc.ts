 ...
 * ## 待拆（602 行）
 *
 * 编译段已切到 `decision-compile.ts`（理由见那里）。剩下的是**形状（类型）**、
 * **解析**，加一段小的**汇总**（`summarize` / `headline` 只是派生视图）。
 * 形状可以独立成文件、让解析更纯 —— 排在 `docs/PLAN-layering-2026-09-21.md` 第五节的队列里。
 *
 ...
 * 代码答的。
 */
const KINDS = ['choice', 'noul', 'score', 'mixed', 'rule'] as const
...
export interface DocBlock {
   ...
   * 候选是每步算出来的，不是这份文件里列的。
   *
   * 选项必须每步重建（AGENTS.md §8.4）—— 固定的候选会让模型去选一个已经
