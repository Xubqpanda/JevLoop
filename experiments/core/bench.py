...
class Benchmark(Protocol):
    ...
    def tool_impls(self) -> dict[str, Callable[..., str]]:
        ...
        ★★ **有状态的环境（ALFWorld 那类）要用下面的 `on_task()`。**
        这个方法的签名里没有任务,所以它没法自己知道该绑定哪个环境实例 ——
        而 runner **每题**都会重新调它一次。
        """
        ...

    def on_task(self, task: Task) -> None:
        """**可选。** runner 在**每题开跑前**调一次,参数是这一题。

        为什么需要它:`tool_impls()` 拿不到任务,而**每题的工具有可能是不同的、
        有状态的** —— ALFWorld 的每一题是一个独立的 TextWorld 环境,
        `step()` 会改变它的状态。没有这个钩子,那种 benchmark 只能靠
        「`tasks()` 和 `tool_impls()` 交替调用」这个**隐式顺序**去猜自己该绑定谁:
        顺序碰巧对了就没事,错了就是「A 题的动作打在 B 题的环境上」,而分数照出。

        ★ 静态数据集（GSM8K / BFCL / BigBench 那些）**不需要实现它** ——
        runner 用的是 `getattr`,没有这个方法就跳过。
        """
        ...
